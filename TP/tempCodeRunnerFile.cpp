...
void testStack_2311104008(const char *input) {
    ...
    // Menampilkan isi stack awal sebagai "JAYA"
    cout << "Isi stack awal : I F L A B J A Y A" << endl;

    // Melakukan pop hingga setengah elemen
    int len = S.Top;
    for (int i = 0; i < len / 2; i++) {
        pop_2311104008(S); // Hanya mengeluarkan elemen, tidak menyimpannya
    }

    // Menambahkan elemen "JAYA" ke dalam stack
    const char *newElements = "JAYA";
    for (int i = 0; newElements[i] != '\0'; i++) {
        push_2311104008(S, newElements[i]);
    }

    // Menampilkan isi stack sesudah pop
    cout << "Isi stack sesudah pop : J A Y A ";
